(async () => {
  ...
  try {
    ...
    if (!Array.isArray(samples) || samples.length < 2 || samples.some((s, i) =>
      ![s.time, s.measured, s.predicted].every(Number.isFinite) || (i && s.time <= samples[i - 1].time))) {
      throw new Error('Invalid force samples');
    }
    const duration = samples.at(-1).time;
    const yMin = Math.min(0, Math.floor(Math.min(...samples.flatMap(s => [s.measured, s.predicted]))));
    const yMax = Math.ceil(Math.max(...samples.flatMap(s => [s.measured, s.predicted])) / 2) * 2;
    ...
    function update() {
      ...
      for (const key of ['measured', 'predicted']) {
